...
void mine_pattern(char *inputEM, char *inputTime) {
	...
	%%{
		...
		action IGNORE_EVENT_EXIT {
			// Main purpose of this action is to nullify all the processing times that were tracked until now because now onwards the actual match would start
			// Eg: trace-> abc M de & tracking for -> 01 M 23 ... in this case event 'a' is causing the overlap problem
			//Get Event Time
			int _ignore_eventTime = getEventTime(tokenCounter);	
			printf("\b_ignore_eventTime %d\n",_ignore_eventTime);
			
			_has_event_entered=0;
		...
		action ACCEPT {
			...
			tempPatternList += (char) fc; //Append the last event. This would not be added because ACCEPT state is called before EVENT state
            matched++;
		...
		ignoreEventSection = (subeventSection %from(IGNORE_EVENT_EXIT));
		
		
		main := ((ignoreEventSection{1} eventSection{2} numberSection+ eventSection{2} ' |') $to(CHUNK) %to(ACCEPT) $lerr(ERR));
